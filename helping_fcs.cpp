	...
	double &rnx, double &rny, double &rnz, voro::voronoicell_neighbor &rc)
{
	...
	// [in]		x,y,z		the coordinates of the particle (the coordinates have to be inside the window)
	...
	// **	a1 = rx + rc.pts[3 * fvert[rfing + 1]]; a2 = ry + rc.pts[3 * fvert[rfing + 1] + 1]; a3 = rz + rc.pts[3 * fvert[rfing + 1] + 2];
		//std::cout << a1 << " " << a2 << " " << a3 << " " << '\n';  //////////////////////////////////////////////////////////
	b1 = rx + rc.pts[3 * fvert[rfing + 2]] * 0.5; b2 = ry + rc.pts[3 * fvert[rfing + 2] + 1] * 0.5; b3 = rz + rc.pts[3 * fvert[rfing + 2] + 2] * 0.5;
	// **	b1 = rx + rc.pts[3 * fvert[rfing + 2]]; b2 = ry + rc.pts[3 * fvert[rfing + 2] + 1]; b3 = rz + rc.pts[3 * fvert[rfing + 2] + 2];
		//std::cout << b1 << " " << b2 << " " << b3 << " " << '\n';  //////////////////////////////////////////////////////////
	c1 = rx + rc.pts[3 * fvert[rfing + 3]] * 0.5; c2 = ry + rc.pts[3 * fvert[rfing + 3] + 1] * 0.5; c3 = rz + rc.pts[3 * fvert[rfing + 3] + 2] * 0.5;
	// **	c1 = rx + rc.pts[3 * fvert[rfing + 3]]; c2 = ry + rc.pts[3 * fvert[rfing + 3] + 1]; c3 = rz + rc.pts[3 * fvert[rfing + 3] + 2];
		//std::cout << c1 << " " << c2 << " " << c3 << " " << '\n';  //////////////////////////////////////////////////////////
	...
	// change of coordinated results in the plane equation ax+by+cz=d-(arx+bry+crz), i.e. ax+by+cz=d*, where d*=d-(arx+bry+crz) ... b1=-d*
	b1 = a * rx + b * ry + c * rz - d;
