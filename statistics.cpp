...
// fc corr_coef computes empirical correlation coefficient of (V_i,V_j), where V_i and V_j are volumes of neighboring cells 
double corr_coef(voro::container &rcon)
{
	...
	voronoicell d;
	std::vector<int> neigh, vert;  // 

	c_loop_all clo(rcon);          // loop over all particlese
...
//				if (nx == rcon.p[ijk][3 * q] && ny == rcon.p[ijk][3 * q + 1] && nz == rcon.p[ijk][3 * q + 2]) {
				if (nx > 0 && nx < 1 && ny > 0 && ny < 1 && nz > 0 && nz < 1) {
					...
					if (barycentrum(x, y, z, nx, ny, nz, a, b)) { // 
	...
	x = round((XY - X*Y / n) *dec) / dec; //
	y = round((X2 - X*X / n)*dec) / dec; //
	z = round((Y2 - Y*Y / n)*dec) / dec; //
...
double corr_coef(voro::container_poly &rcon)
{
	...
	c_loop_all clo(rcon);          // loop over all particles
...
void con_stats(voro::container_poly &con) {
	...
	std::cout << "Number of nonzero radii: " << citac << "\n";
	std::cout << "Average radius: " << rad / n << "\n";
}
...
void cell_stats(voro::container &rcon, const char* con_out)
{
	...
	char outname[100];
	sprintf(outname, "cell_stats%s.txt", con_out);
	FILE *f;
	f = fopen(outname, "w");
	...
	for (j = 0; j < rcon.nxyz; j++) { // loop over boxes
		...
		for (i = 0; i < rcon.co[j]; i++) { // loop over particles in considered box
			...
			fprintf(f, "%d %g %g %g %d %d %g \n",
				...
				citac,						//c.number_of_faces(),		// number of faces = number of neighbors
...
void cell_stats(voro::container_poly &rcon, const char* con_out)
{
	...
	for (j = 0; j < rcon.nxyz; j++) { // loop over boxes
		...
		for (i = 0; i < rcon.co[j]; i++) { // loop over particles in considered box
			...
			grain = rcon.compute_cell(c, j, i);		// computes the cell of the particle given by (j,i)

											// fprintf(f, "%d %g %g %g \n", rcon.id[j][i], con.p[j][3 * i], con.p[j][3 * i + 1], con.p[j][3 * i + 2]);
			if (grain == true) {
				neigh.clear(); c.neighbors(neigh); citac = 0; ins = 1;
				for (k = 0; k < neigh.size(); k++) {
					if (neigh[k] > 0) { citac++; }
					if (neigh[k] < 0) { ins = 0; }
				}
				...
				fprintf(f, "%d %g %g %g %d %d %g %d \n",
					rcon.id[j][i],				// ID
												// cell:
					c.volume(),					// volume
					c.surface_area(),			// surface area
					c.total_edge_distance(),	// total edge length
					citac,						// number of faces		// number of faces = number of neighbours
					c.number_of_edges(),		// number of edges
												// number of vertices = number of edges - number of faces + 2
					c.max_radius_squared(),		// maximal distance of a vertex to the cell centroid squared
					ins							// indicator whether the generator is inside the window
				);
			}
		}
	}
...
void neigh_list(voro::container_poly &rcon, const char* con_out)
{
	...
	char outname[100];
	sprintf(outname, "Nlist%s.txt", con_out);
...
void face_list(voro::container_poly &rcon, const char* con_out)
{
	...
	char outname[100];
	sprintf(outname, "Flist%s.txt", con_out);
	FILE *f;
	f = fopen(outname, "w");

	if (f == NULL) { std::cout << "NELZE zapsat Flist \n"; }
	...
	for (j = 0; j < rcon.nxyz; j++) { // loop over boxes
		...
		for (i = 0; i < rcon.co[j]; i++) { // loop over particles in considered box
			...
			if (grain == true) {
				c.face_areas(fare);
				fprintf(f, "%d ", rcon.id[j][i]);
				fprintf(f, "%d ", fare.size());
				// fprintf(f, "%d %g %g %g \n", rcon.id[j][i], con.p[j][3 * i], con.p[j][3 * i + 1], con.p[j][3 * i + 2]);
				for (k = 0; k < fare.size(); k++) {
...
void face_stats(voro::container &rcon, const char* con_out)
{
	...
	char outname[100];
	sprintf(outname, "face_stats%s.txt", con_out);
	...
	for (j = 0; j < rcon.nxyz; j++) { // loop over boxes
		...
		for (i = 0; i < rcon.co[j]; i++) { // loop over particles in considered box
			...
			rcon.compute_cell(c, j, i);		// computes the cell of the particle given by (j,i)
			c.neighbors(neigh);
			...
			for (k = 0; k < neigh.size(); k++) {	// loop over neighbors = loop over faces
				if (rcon.id[j][i] > neigh[k]) {		// if ID of neighbor is higher than id of current cell continue (to avoid duble counting)
					...
					fprintf(f, "%g %d %g %g %g\n",
						fare[k],		// face area
						ford[k],		// number of vertices per face = number of edges per face
						fper[k],		// face perimeter
						vol,			// volume of the first cell
						vol2			// volume of the second cell
					);
				}
			}
		}
		...
		for (j = 0; j < rc.nu[i]; j++) {	// loop over all neighboring vertices of the given vertex
			...
			// find faces containing this edge:
				...
				for (k = 0; k < rc.number_of_faces(); k++) {
					for (l = 0; l < vert[p]; l++) {
						if (vert[p + 1 + l] == i) { m = 1; }				// the first vertex is there
						if (vert[p + 1 + l] == rc.ed[i][j]) { n = 1; }		// the second vertex is there
						
					}
					p = p + vert[p] + 1;
					if (m*n == 1) { if (e == 0) { f1 = k; e++; } else { f2 = k; } } // both vertices are there
					m = 0; n = 0;
				}
				e = 0;
			// steny nalezeny
...
//computes sphericity of the cell -- vraci moc velka cisla - numericky nestabilni
double sphericity(voro::voronoicell &rc) {
	return (( pow(PI,(double)1/3)*pow(6*rc.volume(),(double)2/3) ) / rc.surface_area());
}
...
// frekvence jsou vypsany do souboru a take vraceny na vystup
void two_point_cf(voro::container_poly &con, std::vector<int> &freq, std::vector<double> &rvalues) {
	// specification: range for r values, number of r values, size of sample

	// vektor rvalues nese hodnoty v kterych se bude 2-point c.f. pocitat

	// range is influenced by window size
	// number of r values should be determined by mean equivalent diameter or more simply by the total number of grains
	//int nr = 30;

	int N = 10000; // size of sample (default 10^5)

	// frec is vector of frequencies
	freq.clear();
	freq.resize(rvalues.size());

	//rvalues.clear();
	//rvalues.resize(nr);

	FILE *f;
	f = fopen("2_point_cf.txt", "w");
	if (f == NULL) { std::cout << "ERROR (two_point_cf): NELZE zapsat \n"; }

	int i, j, id1, id2;
	double x, y, z, sx, sy, sz, cx, cy, cz;

	double r;

	//std::cout << " N: " << N << "\n";
	fprintf(f, "%d %d \n", 0, N);
	for (j = 0; j < rvalues.size(); j++) {
		r = rvalues[j];

		for (i = 0; i < N; i++) {
			x = con.ax+(con.bx - con.ax)*uniform(0,1);		// first location
			y = con.ay+(con.by - con.ay)*uniform(0,1);
			z = con.az+(con.bz - con.az)*uniform(0,1);

			// second location
			sphere_uniform(sx, sy, sz);
			sx = x + r * sx;
			sy = y + r * sy;
			sz = z + r * sz;
			...
			con.find_voronoi_cell(x, y, z, cx, cy, cz, id1);
			con.find_voronoi_cell(sx, sy, sz, cx, cy, cz, id2);
